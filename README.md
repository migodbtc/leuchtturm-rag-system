...
```bash
git clone <repo-url>
cd leuchtturm-rag-system
```

### 2. Pull the Ollama embedding model

Sumerian uses `nomic-embed-text` for embeddings and optionally `qwen2.5:3b` as a local LLM fallback.
...
```bash
# Laravel
cd web && composer install && pnpm install && cd ..

# Leuchtturm FastAPI
cd server && poetry install && cd ..

# Yellowpad server
cd app/server && poetry install && cd ../..
...
WEB_APPLICATION_URL=http://localhost:8000
```

**`web/.env`** — run `php artisan key:generate` after copying, then set:
```env
APP_KEY=          # auto-generated
DB_CONNECTION=sqlite   # default; switch to mysql if needed
...
```bash
# Leuchtturm (SQLite by default)
cd web && php artisan migrate && cd ..

# Yellowpad (MySQL required)
cd app/server && poetry run alembic upgrade head && cd ../..
```

### 6. Start

```bash
# Full stack (Leuchtturm + Yellowpad)
task dev
